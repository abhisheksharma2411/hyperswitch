...
impl UnifiedConnectorServiceError {
    ...
    /// 4. Plain-text fallback — prefix-match on `status.message()` for known patterns
    /// 5. Raw tonic status — catch-all
    pub fn from_grpc_error(status: &tonic::Status, connector_name: &str) -> Self {
        Self::decode_connector_error_response(status, connector_name)
            ...
            .or_else(|| Self::decode_from_plain_text_message(status))
    ...
    /// Fallback: infer error type from known plain-text prefixes in `status.message()`.
    ///
    /// Used when `status.details()` is empty or cannot be proto-decoded (e.g. older UCS versions).
    fn decode_from_plain_text_message(status: &tonic::Status) -> Option<Self> {
        let message = status.message();
        let error_code = UcsErrorCode::try_from(message).ok()?;
        error_code.into_ucs_error(message)
    }
}
...
/// Known plain-text prefixes from UCS `IntegrationError` `#[error("...")]` attributes.
/// Used as fallback when proto details are absent.
///
/// These are derived from the `#[error]` strings on each `IntegrationError` variant in
/// connector-service. Only variants whose messages have a distinctive prefix are listed —
/// the proto path (`decode_integration_error`) handles all variants.
...
impl UcsErrorCode {
    /// (prefix, variant) pairs for plain-text fallback matching.
...
impl<'a> TryFrom<&'a str> for UcsErrorCode {
    type Error = ();

    /// Infer the UCS error code from a plain-text tonic status message.
    /// Checks exact matches first, then prefix matches.
    fn try_from(message: &'a str) -> Result<Self, Self::Error> {
        Self::EXACT_MATCHES
            .iter()
